"""Sigma-Lognormal 적합 — 신경과학(Plamondon 기지어드 운동학, Acien BeCAPTCHA-Mouse) 적용.

사람 기지어드 속도 프로필 v(τ)를 K개 로그노멀 스트로크의 합으로 모델링하고 적합한다:
    v(τ) = Σ_i D_i · Λ(τ; t0_i, μ_i, σ_i)
    Λ(τ; t0, μ, σ) = 1 / (σ (τ−t0) √(2π)) · exp(−(ln(τ−t0) − μ)² / (2σ²)),  τ>t0

**속도 크기(magnitude) 버전**: 원 Sigma-Lognormal은 속도 **벡터**를 방향각(θs,θe)까지 넣어
적합하지만, 여기서는 판별 신호의 핵심인 **속도 프로필 모양**만 잡는 크기 버전이다(방향각 생략).
canonical은 시각지속률 리샘플이라 dt_cv는 죽지만 v(τ) '모양'은 보존되므로 타이밍 없이 적합 가능.

feature(전역 요약, 4차원 [0,1] 스케일 → cond 벡터에 concat):
    [ SNR/30, K/3, (μ_w+6)/7.5, σ_w/1.5 ]
    - SNR(dB): 재구성 품질 = "얼마나 로그노멀로 설명되는지"(사람↔봇↔기계) → 핵심 판별 신호
    - K: 적정 최소 스트로크 수(1~3) = 이동 복잡도(v2는 경유점 수와 상관)
    - μ_w, σ_w: 진폭(D) 가중 평균 로그시간·로그분산(주 스트로크 모양)

적정 K: K=1,2,3 적합 후 **최고 SNR에서 1 dB 이내인 가장 작은 K**(하이퍼모드 → K가 실제로 변별력).
"""
import warnings
...
SNR_MARGIN_DB = 1.0       # 적정 K: 최고 SNR에서 이 이내면 더 작은 K 선택
...
def _init_peaks(v, tau, K):
    ...
    for idx in order:
        ...
        if len(picked) >= K:
            break
    while len(picked) < K:                       # 분리된 봉우리 부족 시 최댓값 재사용
        j = int(np.argmax(v))
...
def _fit_k(v, tau, K):
    ...
    with warnings.catch_warnings():
        ...
        try:
            # trf + 완화된 tolerance + maxfev 제한 = 속도 우선(정밀도보다 SNR 판별에 충분).
...
def sigma_features(speed) -> np.ndarray:
    """속도 프로필(63,)/(63,1) → 4차원 Sigma-Lognormal 요약 feature([0,1] 스케일).

    적정 K(early-stop): K=1 적합 → K=2가 1dB 이상 개선할 때만 K=3까지. 대부분 1~2에서 멈춰
    적합 횟수를 준다(전건 3회 대비 아낌). 선택 규칙은 동일(최고 SNR에서 1dB 이내 최소 K).
    """
    v = np.asarray(speed, dtype=np.float64).ravel()
    n = len(v)
    if n < 4 or float(v.sum()) < 1e-9:            # 정지/해체 → 영벡터
...
def raw_speed(canon) -> np.ndarray:
    """canonical trajectory (63,2) 변위 → 스텝별 속도 크기(63,). Sigma 적합 입력.

    → --lognormal(log 변환)과 무관하게 **원 입력 크기**로 적합한다(로그노멀 적합은 원 프로필에)."""
