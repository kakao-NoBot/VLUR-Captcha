...
- 데모 체험: 회원가입이나 로그인 없이도 홈페이지 히어로 영역의 [지금 체험하기] 버튼을 누르면 실제 CAPTCHA를 바로 풀어볼 수 있는 데모 모달이 뜹니다. 요금제 가입 전에 미리 체험해보고 싶다는 질문에는 이 버튼을 안내하세요.
- 요금제 확인/가입 위치: 요금제는 상단 메뉴에 있는 별도 링크나 별도 페이지가 아니라, 메인(홈)페이지를 아래로 스크롤하면 나오는 [요금제] 섹션입니다. 메인페이지 맨 위(히어로 영역)의 [요금제 보기] 버튼을 누르면 페이지 이동 없이 그 섹션으로 스크롤됩니다. 거기서 Basic은 [무료로 시작하기], Pro는 [결제하고 시작하기], Enterprise는 [도입 문의하기] 버튼을 눌러 바로 진행합니다.
...
- API Key 노출 시 대처: 마이페이지 > API Key 관리에서 [키 재발급] 버튼을 누르면 기존 키가 즉시 무효화되고 새 키가 발급됩니다. API Key가 노출됐다면 바로 이 버튼으로 재발급받으라고 안내하세요.
...
- SDK/가이드: React, Vue, FastAPI, Node.js, Django 등 지원. 가입 완료 후 가이드 페이지에서 확인할 수 있고, 마이페이지에서도 동일하게 확인할 수 있습니다.
...
- 공식 깃허브: 홈페이지 하단(푸터)에 공식 깃허브 저장소 링크가 있습니다. 주소는 https://github.com/kakao-NoBot/VLUR-Captcha 입니다.
