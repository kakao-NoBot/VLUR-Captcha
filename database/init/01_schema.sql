...
CREATE TABLE IF NOT EXISTS users (
    ...
    theme_customization_allowed BOOLEAN NOT NULL DEFAULT TRUE COMMENT '고객이 CAPTCHA 테마를 직접 변경할 수 있는지 여부',
...
CREATE TABLE IF NOT EXISTS api_keys (
    ...
    captcha_theme VARCHAR(24)    NOT NULL DEFAULT 'orange' COMMENT 'CAPTCHA 위젯 색상 프리셋 ID 또는 HEX 색상',
    created_at   DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    expired_at   DATETIME        NULL,
    is_active    BOOLEAN         NOT NULL DEFAULT TRUE,
    PRIMARY KEY (api_key_id),
    UNIQUE KEY uk_api_keys_api_key_hash (api_key_hash),
    UNIQUE KEY uk_api_keys_site_key (site_key),
